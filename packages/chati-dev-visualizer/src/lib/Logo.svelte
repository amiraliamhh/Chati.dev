<script>
	let { size = 32, class: cls = '' } = $props();
</script>

<svg
	xmlns="http://www.w3.org/2000/svg"
	viewBox="0 0 200 200"
	width={size}
	height={size}
	class={cls}
>
	<defs>
		<clipPath id="lc">
			<path
				d="M100,8 C155,8 190,40 190,90 C190,140 155,172 100,172 C82,172 66,167 52,159 L22,192 L34,155 C18,140 10,118 10,90 C10,40 45,8 100,8 Z"
			/>
		</clipPath>
		<radialGradient id="lg" cx="38%" cy="30%" r="68%">
			<stop offset="0%" stop-color="#8EC5F5" />
			<stop offset="40%" stop-color="#5DA2E2" />
			<stop offset="75%" stop-color="#4A90D9" />
			<stop offset="100%" stop-color="#2D6BAF" />
		</radialGradient>
		<linearGradient id="lr" x1="0" y1="0" x2="0" y2="1">
			<stop offset="0%" stop-color="rgba(255,255,255,0.5)" />
			<stop offset="30%" stop-color="rgba(255,255,255,0.15)" />
			<stop offset="100%" stop-color="rgba(0,0,0,0)" />
		</linearGradient>
		<linearGradient id="lbs" x1="0" y1="0" x2="0" y2="1">
			<stop offset="0%" stop-color="rgba(0,0,0,0)" />
			<stop offset="65%" stop-color="rgba(0,0,0,0)" />
			<stop offset="100%" stop-color="rgba(15,30,60,0.25)" />
		</linearGradient>
	</defs>

	<!-- Chat bubble -->
	<path
		d="M100,8 C155,8 190,40 190,90 C190,140 155,172 100,172 C82,172 66,167 52,159 L22,192 L34,155 C18,140 10,118 10,90 C10,40 45,8 100,8 Z"
		fill="url(#lg)"
	/>
	<ellipse cx="82" cy="38" rx="52" ry="25" fill="rgba(255,255,255,0.18)" clip-path="url(#lc)" />
	<path
		d="M100,8 C155,8 190,40 190,90 C190,140 155,172 100,172 C82,172 66,167 52,159 L22,192 L34,155 C18,140 10,118 10,90 C10,40 45,8 100,8 Z"
		fill="url(#lbs)"
		clip-path="url(#lc)"
	/>
	<path
		d="M100,8 C155,8 190,40 190,90 C190,140 155,172 100,172 C82,172 66,167 52,159 L22,192 L34,155 C18,140 10,118 10,90 C10,40 45,8 100,8 Z"
		fill="none"
		stroke="url(#lr)"
		stroke-width="1.5"
	/>

	<!-- Eyes -->
	<ellipse cx="72" cy="95" rx="18" ry="20" fill="#1a1a2e" />
	<ellipse cx="128" cy="95" rx="18" ry="20" fill="#1a1a2e" />
	<ellipse cx="65" cy="86" rx="7" ry="5" fill="rgba(255,255,255,0.55)" />
	<ellipse cx="121" cy="86" rx="7" ry="5" fill="rgba(255,255,255,0.55)" />
	<ellipse cx="77" cy="100" rx="3.5" ry="2.5" fill="rgba(255,255,255,0.3)" />
	<ellipse cx="133" cy="100" rx="3.5" ry="2.5" fill="rgba(255,255,255,0.3)" />
</svg>
